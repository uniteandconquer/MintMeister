...
//        JOptionPane.setDefaultLocale(locale);
        ...
        if(lockInstance(dir + "/mm_instance.lock"))
        {
            if(args.length == 0)
            {
            ...
            switch (args[0])
            {
                ...
                case "-gui":
                    CheckLaunch();      
                    BackgroundService bgs = new BackgroundService(true);  
                    break;
//                case "-cli":
//                    CheckLaunch();
//                    //Implementation pending
//                    //ATTENTION: ADD CONSOLE FEEDBACK
//                    bgs = new BackgroundService(false);
//                    break;
                default:
     ...
     {
         if(System.getProperty("user.dir").endsWith("NetBeansProjects\\MintMeister"))
             return;
         ...
         if(!System.getProperty("user.dir").endsWith("MintMeister"))
        {
            if(new SystemInfo().getOperatingSystem().getFamily().equals("Windows"))
            {
